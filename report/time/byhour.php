<?
	...
	$sql="SELECT HOUR( ec_order.time ) , SUM( ec_product.total ) , SUM( ec_product.amount ) 
FROM ec_order INNER JOIN ec_product ON ec_product.order_id = ec_order.order_id INNER JOIN ec_customer ON ec_customer.customer_id = ec_order.customer_id GROUP BY HOUR( ec_order.time ) ";
	$sql1="SELECT DATE( ec_order.time ) , SUM( ec_product.total ) , SUM( ec_product.amount ) 
FROM ec_order INNER JOIN ec_product ON ec_product.order_id = ec_order.order_id INNER JOIN ec_customer ON ec_customer.customer_id = ec_order.customer_id GROUP BY DATE( ec_order.time ) ";
	$sql2="SELECT DAYOFWEEK( ec_order.time ) , SUM( ec_product.total ) , SUM( ec_product.amount ) 
FROM ec_order INNER JOIN ec_product ON ec_product.order_id = ec_order.order_id INNER JOIN ec_customer ON ec_customer.customer_id = ec_order.customer_id GROUP BY DAYOFWEEK( ec_order.time ) ";
...
<script>
	$(document).ready(function(){
		...
		var s2 = [<? while($rows = mysqli_fetch_array($list_2)){ echo $rows['SUM( ec_product.total )'].",";}?>];
		var s3 = [<? while($rows = mysqli_fetch_array($list_3)){ echo $rows['SUM( ec_product.total )'].",";}?>];
        var ticks1 = [<? while($rows = mysqli_fetch_array($list_1b)){ echo "'".$rows['HOUR( ec_order.time )']."',";}?>];
		var ticks2 = [<? while($rows = mysqli_fetch_array($list_2b)){ echo "'".$rows['DATE( ec_order.time )']."',";}?>];
		var ticks3 = [<? while($rows = mysqli_fetch_array($list_3b)){ echo "'".$rows['DAYOFWEEK( ec_order.time )']."',";}?>];
...
<? while($rows = mysqli_fetch_array($list_3c)){?>
	<tr>
		<td><?=$rows['DAYOFWEEK( ec_order.time )'];?></td>
        <td><?=$rows['SUM( ec_product.total )'];?></td>
    </tr>
<? } ?>
</table>
